...
#    print (M)
#    print (N)
    im_fft = fftpack.fft2(img) #fftpack.fft2(img) #Computes the 2D discrete aperiod fourier transform. Does NOT divide for M*N
...
#    plt.title('Filtered Spectrum')
    ...
    CannyMuff = cv2.Canny(im_muffle,200,300)
...
#            CellInfoCluster[:,j,i] = array
    ...
    CellInfoCluster[4,:,:][CellInfoCluster[5,:,:] < 0.1*ContentThreshold*MaxCellSum] = 0
   ...
   #%% Cluster identification of cells , maybe I could combine both loops into 1 with inverse coords...
    ...
    ExportImages = np.zeros((1,TargetImgH,TargetImgW))
    ...
    if (Clustercount > 1):
        ...
        ExportImages[0,:,:] = cv2.resize(img, (TargetImgW, TargetImgH))
    else:
        ...
        HeaderCoordinates[0,0] = MaxCoordH
        HeaderCoordinates[1,0] = MinCoordW    
        ...
        ExportImages[0,:,:] = ClusterImg
        
        namestring = "Cluster Image " + str(0)
        cv2.imshow(namestring , ExportImages[0,:,:].astype(np.uint8))
