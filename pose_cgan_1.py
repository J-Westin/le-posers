...
from cgan_architectures.cgan_architecture_5 import model_generator, model_discriminator
...
#			X[i,] = np.random.random(self.latent_dim)
			X[i,] = np.random.randint(2, size=self.latent_dim).reshape(1,self.latent_dim)
			# Generate a random direction
			q = np.random.randn(4)
			q = q/np.linalg.norm(q)
			# Generate random direction components
...
def main(batch_size, epochs, version, load_model):
	...
	cgan = CGAN(batch_size, epochs, version, load_model, reps = 2000)
...
parser.add_argument('--version', help='version of the neural network.', default = 12)
