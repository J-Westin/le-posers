...
#	print((int(input_shape[0]/S_T),int(input_shape[1]/S_T),256))
	x = keras.layers.Dense(10*16*1024, activation='relu', name='flatten_g')(x)
	x = keras.layers.Reshape((10,16,1024), name="generator_x")(x)
#	x = keras.layers.Lambda(lambda im:K.resize_images(im,b3r[0],b3r[1], 'channels_last'), name='block3_pool_g')(x)
	x = keras.layers.UpSampling2D(size=(S_4, S_4), name='block4_pool_g', interpolation='bilinear')(x)
	x = keras.layers.Conv2D(512, (5, 5),
					  padding='same',
					  name='block3_conv4_g')(x)
	x = keras.layers.LeakyReLU(0.3)(x)
	x = keras.layers.BatchNormalization()(x)
#	x = keras.layers.ReLU()(x)
	x = keras.layers.UpSampling2D(size=(S_3, S_3), name='block3_pool_g', interpolation='bilinear')(x)
...
#	x = keras.layers.ReLU()(x)
	...
	x = keras.layers.UpSampling2D(size=(S_2, S_2), name='block2_pool_g', interpolation='bilinear')(x)
...
#	x = keras.layers.ReLU()(x)
	...
	x = keras.layers.UpSampling2D(size=(S_1, S_1), name='block1_pool_g', interpolation='bilinear')(x)
...
#	x = keras.layers.BatchNormalization()(x)
	...
	x = keras.layers.UpSampling2D(size=(S_0, S_0), name='block0_pool_g', interpolation='bilinear')(x)
